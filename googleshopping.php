...
$this->id_lang = Configuration::get($this->name.'_lang');
...
$this->id_currency = Configuration::get($this->name.'_currency');
...
{
    $category = new Category($id_category, Configuration::get($this->name.'_lang'));
    ...
    $path = '';
		...
		return $this->_getPath($category->id_parent, $path);
	}
  ...
  private function _processProduct($product)
  {
   ...
   $item_data .= $this->_xmlElement('g:adwords_redirect', $product_link);
    ...
    $items .= "<g:sale_price_effective_date>".Product::getPriceStatic($product['id_product'])."</g:sale_price_effective_date>\n";
    */
  ...
  {
    $price = number_format(Tools::convertPrice(Product::getPriceStatic($id_product, true, $id_product_attrib, 6, NULL, false, false), $this->currencies[$this->id_currency]), 2, '.', '');
  ...
  {
    $price = number_format(Tools::convertPrice(Product::getPriceStatic($id_product, true, $id_product_attrib, 6), $this->currencies[$this->id_currency]), 2, '.', '');
